...
BaseLdflags="$(gcc -print-file-name=crti.o) $(gcc -print-file-name=crt1.o) $(gcc -print-file-name=crtn.o) -lc -lm --dynamic-linker $(readelf -l /bin/sh | grep -oP '(?<=interpreter: )/.*?(?=])') -lpthread -ldl -lz"
