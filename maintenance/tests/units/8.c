...
int main(void) {
    ...
    EventEmitter_action_put_control_flags(&ee, 1, 0);

    EventEmitter_action_on(&ee, "test", cb);

    EventEmitter_action_emit(&ee, "test", NULL);

    EventEmitter_action_put_control_flags(&ee, 0, 0);

    EventEmitter_action_emit(&ee, "test", NULL);

    JsonWriter *jw = jw_open("/tmp/LBEETEST0008.json");
    ...
    gchar *buf;
    g_file_get_contents("/tmp/LBEETEST0008.json", &buf, NULL, NULL);
